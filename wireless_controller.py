...
def main():
    ...
    print(('*'*10 + 'Health Check started for Wireless Controllers {0}' + '*'*10).format(hostname))
    logger1 = setup_logger(os.path.basename(__file__)[:-3])
    logger1.info(('*'*10 + 'Health Check started for Wireless Controllers {0}' + '*'*10).format(hostname))
    ...
    if err:
        ...
        if con_err:
            ...
            logger1.info("Connection Established with Wireless Controllers {0}".format(hostname))
            commands = ["Show ap summary", "Show sysinfo"]
